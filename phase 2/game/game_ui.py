...
class GameUI:
    ...
    def duringGameUI(self, screen, deltaTime):
        ...
        if hasattr(GameManager.Manager, 'level'):
            level = int(GameManager.Manager.level)
            self.renderText(screen, f'Level: {level}', 36, WhiteColor, (10, 40))

        if hasattr(GameManager.Manager, 'score'):
            score = int(GameManager.Manager.score)
    ...
    def gameOverUI(self, screen):
        ...
        if hasattr(GameManager.Manager, 'highScore'):
            highScore = int(GameManager.Manager.highScore)
