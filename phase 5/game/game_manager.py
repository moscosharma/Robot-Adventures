from game.core import GameWorld, getSpawnDistribution, Tags, getRandomPosition, Resources
from game.platform import TileMap, Background
from game.characters import Player, ZombieEnemy, AdventurerEnemy, EnemyTypes
...
class GameManager:
    ...
    def start_game(self):
        ...
        Resources.Sounds['background'].play(-1).set_volume(0.3)
        self.isRunning = True
    ...
    def itemCollected(self, item):
        match(item.tag):
            case Tags.GoldPoint | Tags.SilverPoint | Tags.BronzePoint | Tags.NormalPoint:
                Resources.Sounds['coin'].stop()
                Resources.Sounds['coin'].play().set_volume(0.5)
                self.gameStats.updateScore(item.getPoints())
            case Tags.PlayerExplosive:
                self.playerKilled()
            case Tags.JumpItem:
                Resources.Sounds['power-up'].stop()
                Resources.Sounds['power-up'].play().set_volume(0.5)
                self.enableJump()
    ...
    def handleEnemySpawning(self):
        if self.gameStats.timeElapsed < self.nextEnemyTime or self.enemyCount >= self.maxEnemyAllowed: return
        
        enemyType = random.choice(self.EnemySpawnDistribution)
        enemy = self.spawnEnemy(enemyType)
        self.nextEnemyTime += self.SpawnEnemyTimeGap
        if not enemy: return

        GameWorld.World.addGameObject(enemy)
        self.enemyCount += 1
    ...
    def enemyKilled(self, enemy):
        ...
        match(enemy.tag):
            case Tags.AdventurerEnemy:
                self.gameStats.updateScore(100)
            case Tags.ZombieEnemy:
                self.gameStats.updateScore(200)

        Resources.Sounds['boom'].stop()
        Resources.Sounds['boom'].play()

    def spawnEnemy(self, enemy):
        spawnPosition = self.getPositionToSpawnEnemy()
        match(enemy):
            case EnemyTypes.Adventurer: return AdventurerEnemy(spawnPosition, self.enemyKilled, self.playerKilled)
            case EnemyTypes.Zombie: return ZombieEnemy(spawnPosition, self.player, self.enemyKilled, self.playerKilled)

    def getPositionToSpawnEnemy(self):
        diffFromFirstExtreme = abs(self.player.position.x - ENEMY_SPAWN_EXTREMES[0])
        diffFromSecondExtreme = abs(self.player.position.x - ENEMY_SPAWN_EXTREMES[1])
        if diffFromFirstExtreme < diffFromSecondExtreme:
            return pygame.Vector2(ENEMY_SPAWN_EXTREMES[1], self.player.position.y)
        else:
            return pygame.Vector2(ENEMY_SPAWN_EXTREMES[0], self.player.position.y)